...
public interface VehicleSpecification {
    ...
    static Specification<VehicleEntity> findByName(String name) {
        return ((root, criteriaQuery, criteriaBuilder) -> criteriaBuilder.equal(root.get(VehicleEntity_.NAME_VEHICLE),
    ...
    static Specification<VehicleEntity> byNameAndExample(final String name, final  VehicleEntity vehicle){
        return (root, query, criteriaBuilder) ->{
            final Predicate byName = criteriaBuilder.equal(root.get(VehicleEntity_.NAME_VEHICLE), criteriaBuilder.literal(name));
